...
func newGenerateHTTPTransport(name string, gorillaMux bool, serviceInterface parser.Interface, methods []string) Gen {
	t := &generateHTTPTransport{
		...
		methods:          methods,
		interfaceName:    utils.ToCamelCase(name + "Service"),
		destPath:         fmt.Sprintf(viper.GetString("gk_http_path_format") , utils.GetParentDIr(), utils.ToLowerSnakeCase(name)),
...
func (g *generateGRPCTransportBase) Generate() (err error) {
	...
	if existingGrpc {
		...
		for _, m := range g.allMethods {
			...
			fields = append(fields, jen.Id(n + "Handler").Qual("github.com/go-kit/kit/transport/grpc", "Handler"))
		}
	} else {
		for _, m := range g.serviceInterface.Methods {
			n := utils.ToLowerFirstCamelCase(m.Name)
	...
	pt.appendFunction(
		...
		nil,
		[]jen.Code{
			jen.Id("s*").Qual("google.golang.org/grpc", "Server"),
		},
		[]jen.Code{
		},
		"",
		[]jen.Code{
			jen.Qual(pbImport, "Register" + utils.ToCamelCase(g.name)+"Server").Call(jen.Id("s"),jen.Id("srv")),
		}...,
	)
	g.code.appendFunction(
		"MakeRegisteFunc",
		nil,
		[]jen.Code{
			jen.Id("srv").Qual(pbImport, utils.ToCamelCase(g.name)+"Server"),
		},
		[]jen.Code{
			jen.Qual("github.com/LongMarch7/higo/util/define", "GrpcRegister"),
		},
		"",
		jen.Return(pt.Raw()),
	)
...
func (g *generateGRPCTransport) Generate() (err error) {
	...
	for _, m := range g.serviceInterface.Methods {
		...
		//		nil,
		//		[]jen.Code{
		//			jen.Id("endpoint").Qual("github.com/go-kit/kit/endpoint", "Endpoint"),
		//			jen.Id("options").Index().Qual(
		...
		if !funcFound {
			...
			g.code.appendFunction(
				...
				jen.List(
					...
					jen.Id("rep"),
					jen.Err(),
				).Op(":=").Id(stp).Dot(utils.ToCamelCase(m.Name)+"Handler").Dot("ServeGRPC").Call(
					jen.Id("ctx"),
					jen.Id("req"),
				),
